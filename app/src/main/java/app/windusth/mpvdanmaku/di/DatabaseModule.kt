...
import app.windusth.mpvdanmaku.database.repository.PlaybackStateRepositoryImpl
import app.windusth.mpvdanmaku.database.repository.PlaylistRepository
import app.windusth.mpvdanmaku.database.repository.RecentlyPlayedRepositoryImpl
import app.windusth.mpvdanmaku.domain.playbackstate.repository.PlaybackStateRepository
import app.windusth.mpvdanmaku.domain.recentlyplayed.repository.RecentlyPlayedRepository
...
val MIGRATION_10_11 = object : Migration(10, 11) {
  override fun migrate(db: SupportSQLiteDatabase) {
    db.execSQL(
      """
      CREATE TABLE IF NOT EXISTS `danmaku_match_cache` (
        `filePath` TEXT NOT NULL,
...
val DatabaseModule =
  module {
    ...
    single<MpvDanmakuDatabase> {
      val context = androidContext()
      Room
        .databaseBuilder(context, MpvDanmakuDatabase::class.java, "mpvex.db")
    ...
    single<RecentlyPlayedRepository> {
      RecentlyPlayedRepositoryImpl(get<MpvDanmakuDatabase>().recentlyPlayedDao())
    }

    single { ThumbnailRepository(androidContext()) }

    single {
      app.windusth.mpvdanmaku.database.repository.VideoMetadataCacheRepository(
        context = androidContext(),
        dao = get<MpvDanmakuDatabase>().videoMetadataDao(),
      )
    }
    ...
    single {
      get<MpvDanmakuDatabase>().networkConnectionDao()
    }

    single {
      get<MpvDanmakuDatabase>().bookmarkDao()
    }

    single {
      get<MpvDanmakuDatabase>().danmakuMatchCacheDao()
    }

    single {
      get<MpvDanmakuDatabase>().danmakuCommentCacheDao()
    }

    single {
      app.windusth.mpvdanmaku.repository.NetworkRepository(
        dao = get(),
      )
    }
    ...
    single {
      PlaylistRepository(
        playlistDao = get<MpvDanmakuDatabase>().playlistDao(),
      )
    }
  }
