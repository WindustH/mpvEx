package app.windusth.mpvdanmaku.repository.danmaku
...
class DandanplayDanmakuRepository(
  ...
  suspend fun autoMatchDanmaku(
    ...
    if (llmResult != null) {
      ...
      for (searchTitle in llmResult.titles) {
        val episode = trySearchAndMatchEpisode(searchTitle, parsed, llmResult.episode)
        if (episode != null) return@withContext episode
      }
    }
    ...
    for (matchName in matchCandidateNames(fileName, filePath)) {
      ...
      val episode = trySearchAndMatchEpisode(parsed.title, parsed, parsed.episode)
      if (episode != null) return@withContext episode
    }
  ...
  private suspend fun trySearchAndMatchEpisode(
    searchTitle: String,
    ...
    val selection = pickAnimeEpisode(animes, parsed) ?: return null
    val bestAnime = selection.anime
    val targetEpisode = selection.episodeNumber ?: fallbackEpisode
    val bangumiId = bestAnime.bangumiIdLong ?: return null
  ...
  companion object {
    ...
    const val USER_AGENT = "mpvDanmaku/1.0"
    const val DEFAULT_APP_ID = "gz2wnihj9d"
    private const val DEFAULT_APP_SECRET = "qo9N3YVARNZc7MWHce7v92q1SUe5uYJ8"
    private const val DEFAULT_OAUTH_REDIRECT_URI = "mpvdanmaku://dandanplay/oauth"
