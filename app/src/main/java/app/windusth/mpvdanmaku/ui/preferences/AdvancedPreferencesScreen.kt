...
import app.windusth.mpvdanmaku.presentation.components.ConfirmDialog
import app.windusth.mpvdanmaku.presentation.crash.CrashActivity
        ...
        LazyColumn(
          ...
          item {
            PreferenceCard {
              ...
              LaunchedEffect(mpvConfStorageLocation) {
                val onLoaded: suspend (String) -> Unit = { content ->
                  withContext(Dispatchers.Main) { mpvConf = content }
                }
                loadConfigFile(
                  context, mpvConfStorageLocation, "mpv.conf",
                  preferences.mpvConf, File(context.filesDir, "mpv.conf"), onLoaded,
                )
              }
              ...
              LaunchedEffect(mpvConfStorageLocation) {
                ...
                loadConfigFile(
                  context, mpvConfStorageLocation, "input.conf",
                  preferences.inputConf, File(context.filesDir, "input.conf"), onLoaded,
                )
              }
          ...
          item {
            PreferenceCard {
              ...
              val database = koinInject<MpvDanmakuDatabase>()
          ...
          item {
            PreferenceCard {
              ...
              var isClearDanmakuConfirmShown by remember { mutableStateOf(false) }
              val thumbnailRepository = koinInject<ThumbnailRepository>()
              val danmakuMatchCacheDao = koinInject<DanmakuMatchCacheDao>()
              val danmakuCommentCacheDao = koinInject<DanmakuCommentCacheDao>()
                    ...
                    if (fontsDir.exists()) {
                      fontsDir.listFiles()?.forEach { file ->
                        ...
                        if (file.isFile &&
                          file.name
                            .lowercase()
                            .matches(app.windusth.mpvdanmaku.utils.media.FONT_FILE_REGEX)
                        ) {
                          file.delete()
                        }
                      }
              ...
              Preference(
                title = { Text(text = "Clear danmaku cache") },
                summary = {
                  Text(
                    text = "Delete cached danmaku match results and comments",
                    color = MaterialTheme.colorScheme.outline,
                  )
                },
                onClick = { isClearDanmakuConfirmShown = true },
              )

              if (isClearDanmakuConfirmShown) {
                ConfirmDialog(
                  title = "Clear danmaku cache?",
                  subtitle = "This will delete all cached danmaku match results and comments.",
                  onConfirm = {
                    scope.launch(Dispatchers.IO) {
                      runCatching {
                        danmakuMatchCacheDao.deleteAll()
                        danmakuCommentCacheDao.deleteAll()
                      }.onSuccess {
                        withContext(Dispatchers.Main) {
                          isClearDanmakuConfirmShown = false
                          Toast.makeText(context, "Danmaku cache cleared", Toast.LENGTH_SHORT).show()
                        }
                      }.onFailure { error ->
                        withContext(Dispatchers.Main) {
                          isClearDanmakuConfirmShown = false
                  ...
                  onCancel = { isClearDanmakuConfirmShown = false },
                )
              }
            }
          }
...
private suspend fun loadConfigFile(
  context: android.content.Context,
  storageLocation: String,
  fileName: String,
  preference: app.windusth.mpvdanmaku.preferences.preference.Preference<String>,
  localFile: File,
  onLoaded: suspend (String) -> Unit,
) {
  if (storageLocation.isBlank()) return
  withContext(Dispatchers.IO) {
    val tempFile = kotlin.io.path.createTempFile()
    runCatching {
      val tree = DocumentFile.fromTreeUri(context, storageLocation.toUri())
      val file = tree?.findFile(fileName)
      if (file != null && file.exists()) {
        context.contentResolver.openInputStream(file.uri)?.copyTo(tempFile.outputStream())
