package app.windusth.mpvdanmaku.ui.browser.filesystem
...
import app.windusth.mpvdanmaku.utils.media.OpenDocumentTreeContract
...
import app.windusth.mpvdanmaku.BuildConfig
import app.windusth.mpvdanmaku.domain.browser.FileSystemItem
...
import app.windusth.mpvdanmaku.preferences.MediaLayoutMode
import app.windusth.mpvdanmaku.preferences.preference.collectAsState
import app.windusth.mpvdanmaku.presentation.components.pullrefresh.PullRefreshBox
import app.windusth.mpvdanmaku.ui.browser.cards.FolderCard
import app.windusth.mpvdanmaku.ui.browser.cards.VideoCard
import app.windusth.mpvdanmaku.ui.browser.components.BrowserBottomBar
import app.windusth.mpvdanmaku.ui.browser.components.BrowserTopBar
import app.windusth.mpvdanmaku.ui.browser.dialogs.AddToPlaylistDialog
import app.windusth.mpvdanmaku.ui.browser.dialogs.DeleteConfirmationDialog
import app.windusth.mpvdanmaku.ui.browser.dialogs.FileOperationProgressDialog
import app.windusth.mpvdanmaku.ui.browser.dialogs.FolderPickerDialog
import app.windusth.mpvdanmaku.ui.browser.dialogs.RenameDialog
import app.windusth.mpvdanmaku.ui.browser.dialogs.SortDialog
import app.windusth.mpvdanmaku.ui.browser.dialogs.ViewModeSelector
import app.windusth.mpvdanmaku.ui.browser.dialogs.VisibilityToggle
import app.windusth.mpvdanmaku.ui.browser.selection.rememberSelectionManager
import app.windusth.mpvdanmaku.ui.browser.sheets.PlayLinkSheet
import app.windusth.mpvdanmaku.ui.browser.states.EmptyState
import app.windusth.mpvdanmaku.ui.browser.states.PermissionDeniedState
import app.windusth.mpvdanmaku.ui.utils.LocalBackStack
import app.windusth.mpvdanmaku.utils.media.CopyPasteOps
import app.windusth.mpvdanmaku.utils.media.MediaUtils
import app.windusth.mpvdanmaku.utils.permission.PermissionUtils
 ...
 */
@Serializable
object FileSystemBrowserRootScreen : app.windusth.mpvdanmaku.presentation.Screen {
...
data class FileSystemDirectoryScreen(
  val path: String,
) : app.windusth.mpvdanmaku.presentation.Screen {
  @OptIn(ExperimentalPermissionsApi::class)
  @Composable
  override fun Content() {
...
fun FileSystemBrowserScreen(path: String? = null) {
  ...
  val playerPreferences = koinInject<app.windusth.mpvdanmaku.preferences.PlayerPreferences>()
  ...
  val navigationBarHeight = app.windusth.mpvdanmaku.ui.browser.LocalNavigationBarHeight.current
  ...
  LaunchedEffect(
    ...
    if (isAtRoot) {
      try {
        val mainScreenObj = app.windusth.mpvdanmaku.ui.browser.MainScreen
          ...
          val results = if (isAtRoot) {
            ...
            parentDirectories.forEach { parentPath ->
              try {
                ...
                val parentResults = app.windusth.mpvdanmaku.ui.browser.filesystem.searchRecursively(context, parentPath, searchQuery)
            ...
            items.filterIsInstance<FileSystemItem.Folder>().forEach { storageVolume ->
              try {
                ...
                val rootResults = app.windusth.mpvdanmaku.ui.browser.filesystem.searchRecursively(context, storageVolume.path, searchQuery)
            ...
            val results = app.windusth.mpvdanmaku.ui.browser.filesystem.searchRecursively(context, currentPath, searchQuery)
  ...
  app.windusth.mpvdanmaku.ui.browser.fab.FabScrollHelper.trackScrollForFabVisibility(
    listState = listState,
    gridState = null,
    ...
    Scaffold(
      topBar = {
        if (isSearching) {
          ...
          BrowserTopBar(
            title = if (isAtRoot) {
              stringResource(app.windusth.mpvdanmaku.R.string.app_name)
            } else {
            ...
            onSettingsClick = {
              backstack.add(app.windusth.mpvdanmaku.ui.preferences.PreferencesScreen)
            },
            additionalActions = {
              if (!isAtRoot) {
                IconButton(
            ...
            onInfoClick = if (videoSelectionManager.isInSelectionMode && !folderSelectionManager.isInSelectionMode) {
              {
                ...
                if (video != null) {
                  val intent = Intent(context, app.windusth.mpvdanmaku.ui.mediainfo.MediaInfoActivity::class.java)
          ...
          FloatingActionButtonMenu(
                ...
                ToggleFloatingActionButton(
                  modifier = Modifier
                    .animateFloatingActionButton(
                      visible = !isInSelectionMode && isFabVisible.value && !app.windusth.mpvdanmaku.ui.browser.MainScreen.getPermissionDeniedState(),
            ...
            FloatingActionButtonMenuItem(
              onClick = {
                isFabExpanded.value = false
                coroutineScope.launch {
                  val recentlyPlayedVideos = app.windusth.mpvdanmaku.utils.history.RecentlyPlayedOps.getRecentlyPlayed(limit = 1)
                    ...
                    if (playlistMode) {
                      ...
                      if (startIndex >= 0) {
                        if (allVideos.size == 1) {
                          ...
                          intent.setClass(context, app.windusth.mpvdanmaku.ui.player.PlayerActivity::class.java)
...
suspend fun searchRecursively(
  ...
  try {
    ...
    val items = app.windusth.mpvdanmaku.repository.MediaFileRepository
...
private suspend fun collectVideosRecursively(
  context: Context,
  folderPath: String,
): List<app.windusth.mpvdanmaku.domain.media.model.Video> {
  val videos = mutableListOf<app.windusth.mpvdanmaku.domain.media.model.Video>()
...
private fun playVideosAsPlaylist(
  context: Context,
  videos: List<app.windusth.mpvdanmaku.domain.media.model.Video>,
) {
...
private fun FileSystemBrowserContent(
  ...
  error: String?,
  isAtRoot: Boolean,
  breadcrumbs: List<app.windusth.mpvdanmaku.domain.browser.PathComponent>,
  ...
  onVideoClick: (app.windusth.mpvdanmaku.domain.media.model.Video) -> Unit,
  onVideoLongClick: (app.windusth.mpvdanmaku.domain.media.model.Video) -> Unit,
  onBreadcrumbClick: (app.windusth.mpvdanmaku.domain.browser.PathComponent) -> Unit,
  folderSelectionManager: app.windusth.mpvdanmaku.ui.browser.selection.SelectionManager<FileSystemItem.Folder, String>,
  videoSelectionManager: app.windusth.mpvdanmaku.ui.browser.selection.SelectionManager<app.windusth.mpvdanmaku.domain.media.model.Video, Long>,
  modifier: Modifier = Modifier,
  isInSelectionMode: Boolean = false,
) {
  val gesturePreferences = koinInject<GesturePreferences>()
  val browserPreferences = koinInject<BrowserPreferences>()
  val thumbnailRepository = koinInject<app.windusth.mpvdanmaku.domain.thumbnail.ThumbnailRepository>()
        ...
        Box(
          ...
          LazyColumn(
            ...
            if (!isAtRoot && breadcrumbs.isNotEmpty()) {
              item {
                app.windusth.mpvdanmaku.ui.browser.filesystem.BreadcrumbNavigation(
                  breadcrumbs = breadcrumbs,
            ...
            items(
              ...
              val folderModel = app.windusth.mpvdanmaku.domain.media.model.VideoFolder(
...
fun FileSystemSortDialog(
  ...
  val browserPreferences = koinInject<BrowserPreferences>()
  val appearancePreferences = koinInject<app.windusth.mpvdanmaku.preferences.AppearancePreferences>()
  ...
  SortDialog(
    ...
    onSortTypeChange = { typeName ->
      app.windusth.mpvdanmaku.preferences.FolderSortType.entries.find { it.displayName == typeName }?.let {
    ...
    onSortOrderChange = { isAsc ->
      browserPreferences.folderSortOrder.set(
        if (isAsc) app.windusth.mpvdanmaku.preferences.SortOrder.Ascending
        else app.windusth.mpvdanmaku.preferences.SortOrder.Descending,
      )
    },
    types = listOf(
      app.windusth.mpvdanmaku.preferences.FolderSortType.Title.displayName,
      app.windusth.mpvdanmaku.preferences.FolderSortType.Date.displayName,
      app.windusth.mpvdanmaku.preferences.FolderSortType.Size.displayName,
    ),
    icons = listOf(
      Icons.Filled.Title,
    ...
    getLabelForType = { type, _ ->
      when (type) {
        app.windusth.mpvdanmaku.preferences.FolderSortType.Title.displayName -> Pair("A-Z", "Z-A")
        app.windusth.mpvdanmaku.preferences.FolderSortType.Date.displayName -> Pair("Oldest", "Newest")
        app.windusth.mpvdanmaku.preferences.FolderSortType.Size.displayName -> Pair("Smallest", "Largest")
        else -> Pair("Asc", "Desc")
      }
    },
    ...
    viewModeSelector = ViewModeSelector(
      ...
      isFirstOptionSelected = folderViewMode == app.windusth.mpvdanmaku.preferences.FolderViewMode.AlbumView,
      onViewModeChange = { isFirstOption ->
        browserPreferences.folderViewMode.set(
          if (isFirstOption) {
            app.windusth.mpvdanmaku.preferences.FolderViewMode.AlbumView
          } else {
            app.windusth.mpvdanmaku.preferences.FolderViewMode.FileManager
          },
        )
      },
    ),
