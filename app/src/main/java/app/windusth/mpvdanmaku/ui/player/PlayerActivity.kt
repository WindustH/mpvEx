...
import app.windusth.mpvdanmaku.databinding.PlayerLayoutBinding
...
import app.windusth.mpvdanmaku.ui.player.controls.PlayerControls
import app.windusth.mpvdanmaku.ui.theme.MpvexTheme
import app.windusth.mpvdanmaku.utils.history.RecentlyPlayedOps
import app.windusth.mpvdanmaku.utils.media.HttpUtils
import app.windusth.mpvdanmaku.utils.media.SubtitleOps
import app.windusth.mpvdanmaku.utils.storage.FileTypeUtils
import app.windusth.mpvdanmaku.utils.storage.FileFilterUtils
...
class PlayerActivity :
   ...
   */
  private val playlistRepository: app.windusth.mpvdanmaku.database.repository.PlaylistRepository by inject()
    ...
    lifecycleScope.launch(Dispatchers.IO) {
      runCatching {
        ...
        val siblingFiles = if (launchSource == "video_list" || launchSource == "recently_played_button" || launchSource == "first_video_button") {
          ...
          val videosInFolder =
            app.windusth.mpvdanmaku.repository.MediaFileRepository.getVideosForBuckets(
              context,
              setOf(bucketId)
            )
          val sortedVideos = app.windusth.mpvdanmaku.utils.sort.SortUtils.sortVideos(videosInFolder, videoSortType, videoSortOrder)
          sortedVideos.mapNotNull { video -> files.find { it.absolutePath == video.path } }
        } else {
          files.sortedWith { f1, f2 -> app.windusth.mpvdanmaku.utils.sort.SortUtils.NaturalOrderComparator.DEFAULT.compare(f1.name, f2.name) }
        }
  ...
  companion object {
     ...
     */
    private const val RESULT_INTENT = "app.windusth.mpvdanmaku.ui.player.PlayerActivity.result"
