...
import app.windusth.mpvdanmuku.databinding.PlayerLayoutBinding
...
import app.windusth.mpvdanmuku.ui.player.controls.PlayerControls
import app.windusth.mpvdanmuku.ui.theme.MpvexTheme
import app.windusth.mpvdanmuku.utils.history.RecentlyPlayedOps
import app.windusth.mpvdanmuku.utils.media.HttpUtils
import app.windusth.mpvdanmuku.utils.media.SubtitleOps
import app.windusth.mpvdanmuku.utils.storage.FileTypeUtils
import app.windusth.mpvdanmuku.utils.storage.FileFilterUtils
...
class PlayerActivity :
   ...
   */
  private val playlistRepository: app.windusth.mpvdanmuku.database.repository.PlaylistRepository by inject()
    ...
    lifecycleScope.launch(Dispatchers.IO) {
      runCatching {
        ...
        val siblingFiles = if (launchSource == "video_list" || launchSource == "recently_played_button" || launchSource == "first_video_button") {
          ...
          val videosInFolder =
            app.windusth.mpvdanmuku.repository.MediaFileRepository.getVideosForBuckets(
              context,
              setOf(bucketId)
            )
          val sortedVideos = app.windusth.mpvdanmuku.utils.sort.SortUtils.sortVideos(videosInFolder, videoSortType, videoSortOrder)
          sortedVideos.mapNotNull { video -> files.find { it.absolutePath == video.path } }
        } else {
          files.sortedWith { f1, f2 -> app.windusth.mpvdanmuku.utils.sort.SortUtils.NaturalOrderComparator.DEFAULT.compare(f1.name, f2.name) }
        }
  ...
  companion object {
     ...
     */
    private const val RESULT_INTENT = "app.windusth.mpvdanmuku.ui.player.PlayerActivity.result"
