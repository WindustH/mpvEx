package app.windusth.mpvdanmuku.ui.browser.filesystem
...
import app.windusth.mpvdanmuku.utils.media.OpenDocumentTreeContract
...
import app.windusth.mpvdanmuku.BuildConfig
import app.windusth.mpvdanmuku.domain.browser.FileSystemItem
...
import app.windusth.mpvdanmuku.ui.browser.cards.FolderCard
import app.windusth.mpvdanmuku.ui.browser.cards.VideoCard
import app.windusth.mpvdanmuku.ui.browser.components.BrowserBottomBar
import app.windusth.mpvdanmuku.ui.browser.components.BrowserTopBar
import app.windusth.mpvdanmuku.ui.browser.dialogs.AddToPlaylistDialog
import app.windusth.mpvdanmuku.ui.browser.dialogs.DeleteConfirmationDialog
import app.windusth.mpvdanmuku.ui.browser.dialogs.FileOperationProgressDialog
import app.windusth.mpvdanmuku.ui.browser.dialogs.FolderPickerDialog
import app.windusth.mpvdanmuku.ui.browser.dialogs.RenameDialog
import app.windusth.mpvdanmuku.ui.browser.dialogs.SortDialog
import app.windusth.mpvdanmuku.ui.browser.dialogs.ViewModeSelector
import app.windusth.mpvdanmuku.ui.browser.dialogs.VisibilityToggle
import app.windusth.mpvdanmuku.ui.browser.selection.rememberSelectionManager
import app.windusth.mpvdanmuku.ui.browser.sheets.PlayLinkSheet
import app.windusth.mpvdanmuku.ui.browser.states.EmptyState
import app.windusth.mpvdanmuku.ui.browser.states.PermissionDeniedState
import app.windusth.mpvdanmuku.ui.utils.LocalBackStack
import app.windusth.mpvdanmuku.utils.media.CopyPasteOps
 ...
 */
@Serializable
object FileSystemBrowserRootScreen : app.windusth.mpvdanmuku.presentation.Screen {
...
data class FileSystemDirectoryScreen(
  val path: String,
) : app.windusth.mpvdanmuku.presentation.Screen {
  @OptIn(ExperimentalPermissionsApi::class)
  @Composable
  override fun Content() {
...
fun FileSystemBrowserScreen(path: String? = null) {
  ...
  val playerPreferences = koinInject<app.windusth.mpvdanmuku.preferences.PlayerPreferences>()
  ...
  LaunchedEffect(
    ...
    if (isAtRoot) {
      try {
        val mainScreenObj = app.windusth.mpvdanmuku.ui.browser.MainScreen
          ...
          val results = if (isAtRoot) {
            ...
            parentDirectories.forEach { parentPath ->
              try {
                ...
                val parentResults = app.windusth.mpvdanmuku.ui.browser.filesystem.searchRecursively(context, parentPath, searchQuery)
            ...
            items.filterIsInstance<FileSystemItem.Folder>().forEach { storageVolume ->
              try {
                ...
                val rootResults = app.windusth.mpvdanmuku.ui.browser.filesystem.searchRecursively(context, storageVolume.path, searchQuery)
  ...
  app.windusth.mpvdanmuku.ui.browser.fab.FabScrollHelper.trackScrollForFabVisibility(
    listState = listState,
    gridState = null,
    ...
    Scaffold(
      topBar = {
        if (isSearching) {
          ...
          BrowserTopBar(
            title = if (isAtRoot) {
              stringResource(app.windusth.mpvdanmuku.R.string.app_name)
            } else {
            ...
            onInfoClick = if (videoSelectionManager.isInSelectionMode && !folderSelectionManager.isInSelectionMode) {
              {
                ...
                if (video != null) {
                  val intent = Intent(context, app.windusth.mpvdanmuku.ui.mediainfo.MediaInfoActivity::class.java)
          ...
          FloatingActionButtonMenu(
                ...
                ToggleFloatingActionButton(
                  modifier = Modifier
                    .animateFloatingActionButton(
                      visible = !isInSelectionMode && isFabVisible.value && !app.windusth.mpvdanmuku.ui.browser.MainScreen.getPermissionDeniedState(),
            ...
            FloatingActionButtonMenuItem(
              onClick = {
                isFabExpanded.value = false
                coroutineScope.launch {
                  val recentlyPlayedVideos = app.windusth.mpvdanmuku.utils.history.RecentlyPlayedOps.getRecentlyPlayed(limit = 1)
                    ...
                    if (playlistMode) {
                      ...
                      if (startIndex >= 0) {
                        if (allVideos.size == 1) {
                          ...
                          intent.setClass(context, app.windusth.mpvdanmuku.ui.player.PlayerActivity::class.java)
...
suspend fun searchRecursively(
  ...
  try {
    ...
    val items = app.windusth.mpvdanmuku.repository.MediaFileRepository
...
private suspend fun collectVideosRecursively(
  context: Context,
  folderPath: String,
): List<app.windusth.mpvdanmuku.domain.media.model.Video> {
  val videos = mutableListOf<app.windusth.mpvdanmuku.domain.media.model.Video>()
...
private fun playVideosAsPlaylist(
  context: Context,
  videos: List<app.windusth.mpvdanmuku.domain.media.model.Video>,
) {
...
private fun FileSystemBrowserContent(
  ...
  error: String?,
  isAtRoot: Boolean,
  breadcrumbs: List<app.windusth.mpvdanmuku.domain.browser.PathComponent>,
  ...
  onVideoClick: (app.windusth.mpvdanmuku.domain.media.model.Video) -> Unit,
  onVideoLongClick: (app.windusth.mpvdanmuku.domain.media.model.Video) -> Unit,
  onBreadcrumbClick: (app.windusth.mpvdanmuku.domain.browser.PathComponent) -> Unit,
  folderSelectionManager: app.windusth.mpvdanmuku.ui.browser.selection.SelectionManager<FileSystemItem.Folder, String>,
  videoSelectionManager: app.windusth.mpvdanmuku.ui.browser.selection.SelectionManager<app.windusth.mpvdanmuku.domain.media.model.Video, Long>,
  modifier: Modifier = Modifier,
  isInSelectionMode: Boolean = false,
) {
  val gesturePreferences = koinInject<GesturePreferences>()
  val browserPreferences = koinInject<BrowserPreferences>()
  val thumbnailRepository = koinInject<app.windusth.mpvdanmuku.domain.thumbnail.ThumbnailRepository>()
        ...
        Box(
          ...
          LazyColumn(
            ...
            if (!isAtRoot && breadcrumbs.isNotEmpty()) {
              item {
                app.windusth.mpvdanmuku.ui.browser.filesystem.BreadcrumbNavigation(
                  breadcrumbs = breadcrumbs,
            ...
            items(
              ...
              val folderModel = app.windusth.mpvdanmuku.domain.media.model.VideoFolder(
...
fun FileSystemSortDialog(
  ...
  val browserPreferences = koinInject<BrowserPreferences>()
  val appearancePreferences = koinInject<app.windusth.mpvdanmuku.preferences.AppearancePreferences>()
  ...
  SortDialog(
    ...
    onSortTypeChange = { typeName ->
      app.windusth.mpvdanmuku.preferences.FolderSortType.entries.find { it.displayName == typeName }?.let {
    ...
    onSortOrderChange = { isAsc ->
      browserPreferences.folderSortOrder.set(
        if (isAsc) app.windusth.mpvdanmuku.preferences.SortOrder.Ascending
        else app.windusth.mpvdanmuku.preferences.SortOrder.Descending,
      )
    },
    types = listOf(
      app.windusth.mpvdanmuku.preferences.FolderSortType.Title.displayName,
      app.windusth.mpvdanmuku.preferences.FolderSortType.Date.displayName,
      app.windusth.mpvdanmuku.preferences.FolderSortType.Size.displayName,
    ),
    icons = listOf(
      Icons.Filled.Title,
    ...
    getLabelForType = { type, _ ->
      when (type) {
        app.windusth.mpvdanmuku.preferences.FolderSortType.Title.displayName -> Pair("A-Z", "Z-A")
        app.windusth.mpvdanmuku.preferences.FolderSortType.Date.displayName -> Pair("Oldest", "Newest")
        app.windusth.mpvdanmuku.preferences.FolderSortType.Size.displayName -> Pair("Smallest", "Largest")
        else -> Pair("Asc", "Desc")
      }
    },
    ...
    viewModeSelector = ViewModeSelector(
      ...
      isFirstOptionSelected = folderViewMode == app.windusth.mpvdanmuku.preferences.FolderViewMode.AlbumView,
      onViewModeChange = { isFirstOption ->
        browserPreferences.folderViewMode.set(
          if (isFirstOption) {
            app.windusth.mpvdanmuku.preferences.FolderViewMode.AlbumView
          } else {
            app.windusth.mpvdanmuku.preferences.FolderViewMode.FileManager
          },
        )
      },
    ),
